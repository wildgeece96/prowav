...
class ProWav(object):
    ...
    def check_file(self):
        ...
        for file_path in self.file_paths:
            ...
            wave_file.close()
    def load_wav(self, sr=None, verbose=0):
        """
        ...
        data = []
        if verbose == 0:
            path_iter = self.file_paths
        else:
            path_iter = tqdm(self.file_paths)
        for file_path in path_iter:
            ...
            if sr:
                wave_file.data = librosa.core.resample(wave_file.data.T, orig_sr=wave_file.rate, target_sr=sr).T
